...
export class DashboardComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    const pieChartCtx = document.getElementById('myPieChart') as HTMLCanvasElement;
    const areaChartCtx = document.getElementById('myAreaChart') as HTMLCanvasElement;
  ...
  updatePieChart(): void {
    ...
    if (pieChart) {
      ...
      pieChart.data.labels = [`Leads (${leadsPercentage}%)`, `Alunos (${alunosPercentage}%)`];
      pieChart.data.datasets[0].data = [this.conversasLeads, this.conversasAlunos];
      pieChart.update();
    } else {
      const pieChartCtx = document.getElementById('myPieChart') as HTMLCanvasElement;
      if (pieChartCtx) {
  ...
  loadPerguntasFrequentes(page: number): void {
    this.dashboardService.getPerguntasFrequentes(page, 20).subscribe(data => {
      this.perguntasFrequentesArray = data.content.reduce((acc: { pergunta: string, quantidade: number }[], pergunta: string) => {
  ...
  renderPieChart(ctx: HTMLCanvasElement): void {
    ...
    const pieChart = new Chart(ctx, {
      type: 'doughnut',
      data: {
        labels: [`Leads (${leadsPercentage}%)`, `Alunos (${alunosPercentage}%)`],
        datasets: [{
